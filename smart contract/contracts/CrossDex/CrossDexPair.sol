...
contract CrossDexPair is CrossDexERC20 {
    ...
    address public immutable routerMainChain;
    uint256 public reserve0;
    uint256 public reserve1;

    modifier onlyRouterMainChain() {
        require(routerMainChain == msg.sender, "Ownable: caller is not RouterMainChain");
        _;
    }

    modifier onlyAxelra() {
        require(msg.sender == axelraBridgeAddress ,"Ownable: caller is not axelraBridge");
        _;
    }

    constructor(address _token0, address _token1,address _routerMainChain)  {
        token0 = IERC20(_token0);
        token1 = IERC20(_token1);
        routerMainChain = _routerMainChain;
    }

    function mint(address to, uint256 amount) public onlyAxelra {
        _mint(to, amount);
    }
    function burn(address to, uint256 amount) public onlyAxelra {
         _burn(to, amount); 
    }
    ...
    function swap(uint256 _amountIn,uint256 _amountOutMin, address _tokenIn,address to) external onlyRouterMainChain returns (uint256 amountOut) {
    ...
    function addLiquidity(uint256 _amount0, uint256 _amount1,address to,bool isForceAdd) external onlyRouterMainChain returns (uint256 liquidity) {
    ...
    function removeLiquidity(uint256 liquidity, address to) external onlyRouterMainChain returns (uint256 amount0, uint256 amount1) {
